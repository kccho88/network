...
- **🤖 ChatGPT AI 통합**: OpenAI GPT API를 사용하여 지능형 설정 스크립트 자동 생성
- **다중 장비 지원**: Cisco, Arista, Alcatel-Lucent, HP, Juniper, Fortinet 장비 설정 스크립트 생성
- **웹 인터페이스**: 브라우저에서 간편하게 사용 가능한 웹 UI 제공
- **명령줄 인터페이스**: CLI를 통한 설정 파일 생성 (템플릿 기반)
- **API 키 관리**: 브라우저에 API 키 자동 저장 및 관리
...
- Python 3.7 이상
- OpenAI API 키 (ChatGPT 사용을 위해 필요)
- Jinja2 3.1.2 이상
- Flask 3.0.0 이상 (웹 버전용)
- OpenAI Python 라이브러리 1.0.0 이상 (웹 버전용)
...
1. **ChatGPT API 키 입력**: 페이지 상단에 OpenAI API 키를 입력합니다
   - API 키는 브라우저에 자동 저장됩니다
   - [OpenAI API 키 발급받기](https://platform.openai.com/api-keys)
2. **제조사 선택**: 드롭다운에서 원하는 제조사를 선택합니다 (Cisco, Arista, Alcatel-Lucent, HP, Juniper, Fortinet)
3. **제품명 입력**: 장비의 호스트 이름을 입력합니다 (예: SW-HQ-01)
4. **네트워크 정보 입력**: 관리 IP 주소와 서브넷 마스크를 입력합니다
5. **추가 설정**: 제조사에 따라 필요한 추가 정보를 입력합니다
   - Cisco/Arista/Alcatel/HP/Juniper: VLAN ID, 관리 인터페이스
   - Fortinet: 관리 포트
6. **생성 버튼 클릭**: ChatGPT AI가 설정 스크립트를 자동으로 생성합니다
7. **복사 또는 다운로드**: 생성된 설정을 복사하거나 파일로 다운로드할 수 있습니다

### ChatGPT AI 통합

웹 버전은 OpenAI의 ChatGPT API를 사용하여 설정 스크립트를 생성합니다:
- **지능형 생성**: AI가 제조사별 표준 명령어 형식을 이해하고 생성
- **맞춤형 설정**: 입력한 정보에 맞춰 최적화된 설정 스크립트 생성
- **정확성**: 실제 장비에 적용 가능한 정확한 CLI 명령어 생성
